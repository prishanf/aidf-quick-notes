...
- **`release/<version>` → `main`**: requires production approval (every release, see [quality-gates.md](quality-gates.md)) plus the QA sign-off gathered on the release branch's QA deployment. On merge, create an **annotated** git tag whose name equals the release notes `version` (and matches the `release/<version>` cut) on the merge tip, then push that tag. Do not move or force-update an existing tag that points elsewhere. Back-merge `main` into `develop` (or re-merge the release branch into `develop`) so hotfix-free history stays consistent.
- **`hotfix/<issue>-<slug>` → `main`**: maintainer approval and, for anything beyond a trivial fix, the same checks a normal release requires. After merge, tag the tip with the hotfix release notes `version` (annotated, pushed, no tag rewrite) and back-merge to `develop` immediately after.
