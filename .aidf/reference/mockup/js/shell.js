...
const SCREENS = [
  ...
  { href: "list.html", label: "Records" },
  { href: "detail.html", label: "Edit record" },
];
...
export function renderFailure(host, err) {
  if (err && err.code === 403) {
    renderError(host, {
      ...
      body: "Your account can view reports but not individual records. Ask a workspace owner to grant the Records role.",
      code: "403",
      retry: false,
    });
    return;
  ...
  renderError(host, {
    ...
    body: "The request did not complete. Nothing was changed. Try again, and if it keeps failing, contact support with the reference below.",
    code: "ERR-FIXTURE",
  });
}
